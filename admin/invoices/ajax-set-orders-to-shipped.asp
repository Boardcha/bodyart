...
rsGetInvoice.Source = "SELECT ship_code, shipped, ID, customer_first, customer_last, email, date_sent, UPS_tracking, USPS_tracking, shipping_type, pay_method, transactionID FROM sent_items WHERE ship_code = 'paid' AND (Review_OrderError <> 1 OR  Review_OrderError IS NULL) AND (shipped = 'Pending shipment') ORDER BY ID DESC"
