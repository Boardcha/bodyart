...
<%
if request.querystring("shipper") = "ups" then
	sql_shipper = " AND shipping_type LIKE '%UPS%'"
end if
if request.querystring("shipper") = "usps" then
	sql_shipper = " AND shipping_type LIKE '%USPS%'"
end if

set objCmd = Server.CreateObject("ADODB.command")
objCmd.ActiveConnection = MM_bodyartforms_sql_STRING
objCmd.CommandText = "SELECT * FROM sent_items  WHERE usps_base64_shipping_label IS NULL AND ship_code = 'paid' AND (Review_OrderError <> 1 OR  Review_OrderError IS NULL) AND shipped = 'Pending shipment' " & sql_shipper & " ORDER BY PackagedBy, CASE WHEN shipping_type LIKE '%office%' THEN 1 WHEN autoclave = 1 THEN 2 WHEN shipping_type LIKE '%express%' THEN 4 WHEN shipping_type LIKE '%ups%' THEN 5 WHEN shipping_type LIKE '%USPS Priority mail%' THEN 6 WHEN (sent_items.shipping_type = 'USPS First Class Mail') THEN 7 WHEN (sent_items.shipping_type LIKE '%global basic%') THEN 8 WHEN shipping_type LIKE '%max%' THEN 9 WHEN   (sent_items.shipping_type = 'DHL Basic mail') THEN 10 WHEN  (sent_items.shipping_type = 'DHL GlobalMail Packet Priority') THEN 11 WHEN  (sent_items.shipping_type = 'DHL GlobalMail Parcel Priority') THEN 12 ELSE 20 END ASC, ID ASC"
