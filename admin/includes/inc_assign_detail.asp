...
if request.form("detailid") <> "" then ' check to see if a detailid is provided and if not, just update the main products table	
    ...
    Set objCmd = Server.CreateObject ("ADODB.Command")
    objCmd.ActiveConnection = DataConn
    objCmd.CommandText = "INSERT INTO tbl_edits_log (user_id, edit_date, detail_id, description) VALUES(?, GETDATE(), ?, 'Automated - Inventory count limited bin - Form at bottom of page to assign detail - Manually updated qty from " & rsGetCurrentQty("qty") & " to ' + ?)"
