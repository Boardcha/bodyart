...
if request.form("detailid") <> "" then ' check to see if a detailid is provided and if not, just update the main products table	
	...
	set objCmd = Server.CreateObject("ADODB.Command")
	objCmd.ActiveConnection = DataConn
	objCmd.CommandText = "INSERT INTO tbl_edits_log (user_id, detail_id, description, edit_date) VALUES (?, " & request.form("detailid") & ",'Automated - Updated / Overwrote qty to " & request.form("qty") & " using the inventory count page for limited bins','" & now() & "')"
