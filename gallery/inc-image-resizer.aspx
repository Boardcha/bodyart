...
private void Page_Load(object sender, System.EventArgs e)
{
	...
	bool debugMode = ((_debugCode.Length > 0) && (Request.QueryString["debug"] + "").Equals(_debugCode));
	...
	if (debugMode)
		Response.Write("Got image path: " + strImagePath + "<br />");
	
	if (strImagePath.StartsWith("/") || Request.QueryString["virtual"] == "1")
	{
		if (debugMode)
			Response.Write("Parsing virtual path to actual disk path..<br />");
		try
		{
			strImagePath = Server.MapPath(strImagePath);
		}
		catch
		{
			if (debugMode)
				Response.Write("Failed to map the virtual path, probably missing folder?<br />");
		}
	}

	if (!System.IO.File.Exists(strImagePath))
	{
		Response.Write("image file " + (debugMode ? "'" + strImagePath + "' " : "") + "does not exist (check ASPNET permissions)<br />");
		if (debugMode)
		{
			System.Collections.Generic.List<string> temp = new System.Collections.Generic.List<string>( strImagePath.Split(new char[] { '/', '\\' }));
			temp.RemoveAt(temp.Count - 1);
			string strFolderPath = string.Join("\\", temp.ToArray());
			Response.Write("Checking parent folder '" + strFolderPath + "'...<br />");
			if (System.IO.Directory.Exists(strFolderPath))
			{
				Response.Write("Parent folder contains " + System.IO.Directory.GetFiles(strFolderPath).Length + " files<br />");
			}
			else
			{
				Response.Write("Parent folder does not exist<br />");
				strFolderPath = Server.MapPath(".");
				Response.Write("Current directory path is '" + strFolderPath + "... exists? " + System.IO.Directory.Exists(strFolderPath));
			}
		}
		Response.End();
	}
	...
	if (debugMode)
		Response.Write("Image file exists!<br />");

	if (strThumbnailPath.Length == 0)
	{
		if (debugMode)
			Response.Write("Missing thumbnail path, using default..<br />");
		strThumbnailPath = System.IO.Path.GetDirectoryName(strImagePath) + System.IO.Path.DirectorySeparatorChar + System.IO.Path.GetFileNameWithoutExtension(strImagePath) + "_thumb" + System.IO.Path.GetExtension(strImagePath);
		if (debugMode)
			Response.Write("Thumbnail path is now: " + strThumbnailPath + "<br />");
	}
