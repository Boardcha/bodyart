...
If request.form("review_edit") = "yes" Then
	set objCmd = Server.CreateObject("ADODB.command")
	objCmd.ActiveConnection = DataConn
	objCmd.CommandText = "UPDATE TBLReviews SET review_rating = ?, status = ?, review_edited = ?, customer_edit_date = DATEADD(dd, 0, DATEDIFF(dd, 0, GETDATE())) WHERE ReviewID = ?" 
	...
	objCmd.Parameters.Append(objCmd.CreateParameter("@review_edited",200,1,2000, var_cleaned_review))
	objCmd.Parameters.Append(objCmd.CreateParameter("@ID",3,1,20, rsCheckDupe.Fields.Item("ReviewID").Value))
	objCmd.Execute()
...
Else
	if rsCheckDupe.eof then ' if there's no review already found for that order detail # then insert a new row
		  ...
		  objCmd.Parameters.Append(objCmd.CreateParameter("@Ratingg",3,1,12, request.form("rating")))
		  objCmd.Parameters.Append(objCmd.CreateParameter("@DateSubmitted",200,1,20, Cstr(date())))
		  objCmd.Parameters.Append(objCmd.CreateParameter("@status",200,1,20, "pending"))
		  ...
		  set objCmd = Server.CreateObject("ADODB.command")
		  objCmd.ActiveConnection = DataConn
		  objCmd.CommandText = "UPDATE TBLReviews SET review = ?, review_rating = ?, status = ? WHERE ReviewID = ?" 
		  objCmd.Parameters.Append(objCmd.CreateParameter("@Review",200,1,2000, var_cleaned_review))
		  objCmd.Parameters.Append(objCmd.CreateParameter("@Ratingg",3,1,12, request.form("rating")))
		  objCmd.Parameters.Append(objCmd.CreateParameter("@status",200,1,20, "pending"))
