...
%>
<%
	page_title = "Your account - Address book"
...
<% else %>
		...
		<span id="address-type" data-type="shipping"></span>
		<button class="btn btn-warning btn d-block mb-4" id="add-address" type="button" text-danger data-status="add" data-url="ajax-cim-add-address.asp" data-header="Add a new address" data-buttonText="Add address" data-toggle="modal" data-target="#updateAddress"><i class="fa fa-plus"></i> Add a new address</button>
		...
		<span id="show-new"></span>	
			...
			End if
			%>
			<div class="card <%= rsGetShippingAddresses.Fields.Item("cim_shippingid").Value %>-block d-inline-block m-md-2 my-2 account-cards">
					<div class="card-body bg-light">
						<% if rsGetShippingAddresses.Fields.Item("default_shipping").Value = 1 then %>
					 	 <h5 class="card-title" id="default-address">DEFAULT ADDRESS</h5>
					  <% end if %>
					  <p class="card-text <%= strShipping_ID %>-address">
						...
						<!-- begin card links -->
					  <span class="card-link pointer text-primary edit-<%= strShipping_ID %> btn-edit" text-danger data-id="<%= strShipping_ID %>" data-first="<%= strShipping_first %>" data-last="<%= strShipping_last %>" data-company="<%= strShipping_company %>" data-address="<%= strShipping_address %>" data-address2="<%= strShipping_address2 %>" data-city="<%= strShipping_city %>" data-state="<%= Replace(strShipping_state,"|","") %>" data-zip="<%= strShipping_zip %>" data-country="<%= strShipping_country %>" data-status="update" data-url="ajax-cim-update-address.asp" data-header="Update address" data-buttonText="Update address" data-toggle="modal" data-target="#updateAddress">
							Edit
						</span>
						

						  <i class="card-link pointer text-primary fa fa-trash-alt delete-address"  data-id="<%= rsGetShippingAddresses.Fields.Item("cim_shippingid").Value %>" data-address="<%= strShipping_address %><br><%= strShipping_address2 %>" data-toggle="modal" data-target="#deleteModal"></i>
				
					<% if rsGetShippingAddresses.Fields.Item("default_shipping").Value = 0 then %>
								<span class="card-link pointer text-primary make-default" data-id="<%= rsGetShippingAddresses.Fields.Item("ID").Value %>">
									Make default
								</span>
								<% end if %>
								<!-- end card links -->
...
<div class="modal fade" id="deleteModal" tabindex="-1" role="dialog" aria-labelledby="headDelete" aria-hidden="true">
		<div class="modal-dialog modal-sm" role="document">
		  <div class="modal-content">
			<div class="modal-header">
			  <h5 class="modal-title" id="headDelete">Delete Address</h5>
			  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
				<span aria-hidden="true">&times;</span>
			  </button>
			</div>
