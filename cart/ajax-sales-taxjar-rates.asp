...
If request("tax_state") = "CO" Then 
	var_retail_delivery_fee = 0.27
Else
	var_retail_delivery_fee = 0
End If
...
if request("state_taxed") = "yes" then
	...
	amount_to_collect = amount_to_collect + var_retail_delivery_fee
	session("amount_to_collect") = session("amount_to_collect") + var_retail_delivery_fee
	...
	if amount_to_collect <> 0 then
		Session("var_salesTax") = amount_to_collect
	Else
		Session("var_salesTax") = 0
	end if
