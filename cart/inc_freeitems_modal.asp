...
' ------- Get FREE items for TIER 1 (ORINGS)
		...
		objCmd.CommandText = "SELECT DISTINCT FlatProducts.min_gauge, FlatProducts.max_gauge, jewelry.title, jewelry.picture,jewelry.ProductID, jewelry.picture, CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END " & _
							...
							"WHERE ProductDetails.free <= 30 AND (free_item_start_date <= GETDATE() OR free_item_start_date is Null) AND (free_item_expiration_date > GETDATE() OR free_item_expiration_date is Null) AND (jewelry.ProductID = 530 OR (jewelry.ProductID = 1649 AND ProductDetails.Gauge <> '1-1/8" & """" & "') OR jewelry.ProductID = 15385) " & _
...
' ------- Get FREE items for TIER 2 (STICKERS)
		...
		objCmd.CommandText = "SELECT ProductDetails.detail_code, ProductDetails.free, 1 As Free_QTY, jewelry.picture, ProductDetails.ProductDetailID, ProductDetails.ProductDetail1, FlatProducts.min_gauge, FlatProducts.max_gauge, jewelry.title, jewelry.picture,jewelry.ProductID, jewelry.picture, CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END, ISNULL(ProductDetails.gauge,'') + ' ' + ISNULL(ProductDetails.Length,'') + ' ' + ISNULL(ProductDetails.ProductDetail1,'') + ' ' + ISNULL(jewelry.title,'') AS 'free_title' " & _
						...
						"WHERE (jewelry.ProductID = 3928) AND (ProductDetails.qty > 0) AND (ProductDetails.free <> 0) AND (ProductDetails.free IS NOT NULL) AND (free_item_start_date <= GETDATE() OR free_item_start_date is Null) AND (free_item_expiration_date > GETDATE() OR free_item_expiration_date is Null) AND (ProductDetails.active = 1) " & _
...
' ------- Get FREE items for TIER 3
		...
		objCmd.CommandText = "SELECT DISTINCT FlatProducts.min_gauge, FlatProducts.max_gauge, jewelry.title, jewelry.picture,jewelry.ProductID, jewelry.picture, CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END " & _
							"FROM ProductDetails INNER JOIN jewelry ON ProductDetails.ProductID = jewelry.ProductID " & _ 
							"INNER JOIN FlatProducts ON FlatProducts.ProductID = jewelry.ProductID " & _ 
							"WHERE ProductDetails.free <= 30 AND (jewelry.ProductID <> 3704) AND (ProductDetails.qty > 0) AND (ProductDetails.free <> 0) AND (ProductDetails.free IS NOT NULL) AND (free_item_start_date <= GETDATE() OR free_item_start_date is Null) AND (free_item_expiration_date > GETDATE() OR free_item_expiration_date is Null) AND (ProductDetails.free IS NOT NULL) AND (ProductDetails.active = 1) " & _
							"ORDER BY CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END, jewelry.title"
...
' ------- Get FREE items for TIER 4
		...
		objCmd.CommandText = "SELECT DISTINCT FlatProducts.min_gauge, FlatProducts.max_gauge, jewelry.title, jewelry.picture,jewelry.ProductID, jewelry.picture, CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END " & _
							"FROM ProductDetails INNER JOIN jewelry ON ProductDetails.ProductID = jewelry.ProductID " & _ 
							"INNER JOIN FlatProducts ON FlatProducts.ProductID = jewelry.ProductID " & _ 
							"WHERE ProductDetails.free <= 50 AND (jewelry.ProductID <> 3704) AND (ProductDetails.qty > 0) AND (ProductDetails.free <> 0) AND (ProductDetails.free IS NOT NULL) AND (free_item_start_date <= GETDATE() OR free_item_start_date is Null) AND (free_item_expiration_date > GETDATE() OR free_item_expiration_date is Null) AND (ProductDetails.free IS NOT NULL) AND (ProductDetails.active = 1) " & _
							"ORDER BY CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END, jewelry.title"
...
' ------- Get FREE items for TIER 5
		...
		objCmd.CommandText = "SELECT DISTINCT FlatProducts.min_gauge, FlatProducts.max_gauge, jewelry.title, jewelry.picture,jewelry.ProductID, jewelry.picture, CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END " & _
							"FROM ProductDetails INNER JOIN jewelry ON ProductDetails.ProductID = jewelry.ProductID " & _ 
							"INNER JOIN FlatProducts ON FlatProducts.ProductID = jewelry.ProductID " & _ 
							"WHERE ProductDetails.free <= 75 AND (jewelry.ProductID <> 3704) AND (ProductDetails.qty > 0) AND (free_item_start_date <= GETDATE() OR free_item_start_date is Null) AND (ProductDetails.free <> 0) AND (ProductDetails.free IS NOT NULL) AND (ProductDetails.free_item_expiration_date > GETDATE() OR ProductDetails.free_item_expiration_date is Null) AND (ProductDetails.free IS NOT NULL) AND (ProductDetails.active = 1) " & _
							"ORDER BY CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END, jewelry.title"
...
' ------- Get FREE items for TIER 6
		...
		objCmd.CommandText = "SELECT DISTINCT FlatProducts.min_gauge, FlatProducts.max_gauge, jewelry.title, jewelry.picture,jewelry.ProductID, jewelry.picture, CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END " & _
							"FROM ProductDetails INNER JOIN jewelry ON ProductDetails.ProductID = jewelry.ProductID " & _ 
							"INNER JOIN FlatProducts ON FlatProducts.ProductID = jewelry.ProductID " & _ 
							"WHERE ProductDetails.free <= 100 AND (jewelry.ProductID <> 3704) AND (ProductDetails.qty > 0) AND (ProductDetails.free <> 0) AND (ProductDetails.free IS NOT NULL) AND (free_item_start_date <= GETDATE() OR free_item_start_date is Null) AND (free_item_expiration_date > GETDATE() OR free_item_expiration_date is Null) AND (ProductDetails.free IS NOT NULL) AND (ProductDetails.active = 1) " & _
							"ORDER BY CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END, jewelry.title"
...
' ------- Get FREE items for TIER 7
		...
		objCmd.CommandText = "SELECT DISTINCT FlatProducts.min_gauge, FlatProducts.max_gauge, jewelry.title, jewelry.picture,jewelry.ProductID, jewelry.picture, CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END " & _
							"FROM ProductDetails INNER JOIN jewelry ON ProductDetails.ProductID = jewelry.ProductID " & _ 
							"INNER JOIN FlatProducts ON FlatProducts.ProductID = jewelry.ProductID " & _ 
							"WHERE ProductDetails.free <= 150 AND (jewelry.ProductID <> 3704) AND (ProductDetails.qty > 0) AND (ProductDetails.free <> 0) AND (ProductDetails.free IS NOT NULL) AND (free_item_start_date <= GETDATE() OR free_item_start_date is Null) AND (free_item_expiration_date > GETDATE() OR free_item_expiration_date is Null) AND (ProductDetails.free IS NOT NULL) AND (ProductDetails.active = 1) " & _
							"ORDER BY CASE WHEN jewelry.ProductID = 2890 THEN '1' ELSE jewelry.ProductID END, jewelry.title"
